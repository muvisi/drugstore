...
@Component({
  selector: 'app-claims-check',
  templateUrl: './claims-check.component.html',
  styleUrls: ['./claims-check.component.scss']
})
export class ClaimsCheckComponent implements OnInit {
  ...
  Columns: string[] = ['sn','member_no','patient_number','visit_number','insurance','visit_type','amount','cashier','confirm','missing','view']
  pendingColumns: string[] = ['sn','member_no','patient_number','visit_number','insurance','visit_type','amount','cashier','confirm','missing']
  ...
  searchingPending(text){
    this.service.searchClaimChecking(text).subscribe((res)=>{
      this.dataSource = new MatTableDataSource(res.results);
    })
  }
  searchStatus(text){
    this.service.searchClaimStatus('RETURNED',text).subscribe((res)=>{
  ...
  getClaims(){
    this.service.getClaimChecking().subscribe((res)=>{
      this.dataSource = new MatTableDataSource(res.results);
    })
  }
  returnedClaims(){
    this.service.getClaimStatus('RETURNED').subscribe((res)=>{
      this.returned = new MatTableDataSource(res.results);
    })
  }
  completedClaims(){
    this.service.getCompleted().subscribe((res)=>{

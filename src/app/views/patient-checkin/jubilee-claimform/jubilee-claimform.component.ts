...
export class JubileeClaimformComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.signatureService.socket().subscribe((res)=>{
      ...
      if (this.signature_type=="staff"){    
        ...
        if(this.signature_type=="staff" && this.signature2_src.search("png")>-1){
          ...
          this.loading=true;
          this.claimformService.signatureJubileeform({visit_number:this.form_data.visit_number,signature:this.signature2_src,type:"PRACTITIONER"}).subscribe((res)=>{
            this.loading=false;
    ...
    this.service.getInsurance(this.route.snapshot.params.id).subscribe((res: any)=>{
      ...
      if(res.insuranceVisit.visit_number!="" && res.insuranceVisit.visit_number!=null){
        this.JubileeInsurance(res.insuranceVisit.visit_number);
      }
  ...
  printPage() {
    if(this.form_data.visit_number!=null){
      document.title=this.form_data.visit_number.concat("-01")
  ...
  Update(){
    ...
    this.claimformService.UpdateJubileeInsurance(this.form_data).subscribe((res)=>{
      this.loading=false;
...
JubileeInsurance(no){
  this.claimformService.JubileeInsurance(no).subscribe(
    (res: any)=>{
